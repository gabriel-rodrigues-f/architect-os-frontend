...
function ProfessionalsComparison() {
  ...
  const series: EvolutionSeries[] = professionals.map((professional) => ({
    key: professional.id,
  ...
  const averagesByProfessional = new Map(
    professionals.map((professional) => [
      ...
      new Map(
        sel
          .capabilityAverages(professional.id)
          .map((measure) => [measure.capability.id, measure.avg]),
      ),
    ]),
  );
                  ...
                  <tbody>
                    {store.capabilities.map((capability) => (
                      <tr key={capability.id}>
                        ...
                        {professionals.map((professional) => {
                          const avg = averagesByProfessional
                            .get(professional.id)
                            ?.get(capability.id);
                          return (
                            <td key={professional.id} className="min-w-[52px]">
                              <LevelHeatCell
