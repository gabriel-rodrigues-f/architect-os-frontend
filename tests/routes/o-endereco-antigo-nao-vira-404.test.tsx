 ...
 *
 * `/settings` deixou de ser um item de menu — a coluna passou a ter as
 * fatias —, mas continua sendo um ENDEREÇO que existe. A primeira fatia era a
 * Elegibilidade; ela saiu do produto no mesmo dia, e o endereço passou a
 * levar à que sobrou na frente: Réguas e limiares. A rota não guarda alcance
 * nenhum: quem decide quem entra é o destino.
 */
describe("o endereço antigo dos Critérios de Progressão", () => {
  ...
  it("leva à primeira fatia, substituindo o histórico em vez de empilhar", () => {
    ...
    expect(Componente().props).toMatchObject({ to: "/scoring-rulers", replace: true });
  });
  ...
  it("saiu da coluna: o menu aponta para as fatias, não para o endereço antigo", () => {
    const destinos = NAV_GROUPS.flatMap((grupo) => grupo.items).map((item) => item.to);
    expect(destinos).not.toContain("/settings");
    expect(destinos).toContain("/scoring-rulers");
  });
});
