...
import { Route as CareerFileRoute } from "@/routes/architects.$architectId";
import { careerFileRouter } from "./ficha-router";
import { renderWithApp } from "./render-app";

/**
 * A FICHA NOS TESTES — [FA-08]. A rota-pai `architects.$architectId` virou o
...
/**
 * Monta a ficha da pessoa `architectId` aberta na aba `tab`, com `page`
 * (o componente da aba) dentro do `<Outlet />` do layout.
 */
export function renderCareerFile(
  page: ReactNode,
  { tab = "overview", architectId = "ana" }: { tab?: CareerFileTab; architectId?: string } = {},
): ReturnType<typeof renderWithApp> {
  careerFileRouter.set({ tab, architectId, outlet: page });
