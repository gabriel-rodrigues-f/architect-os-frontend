...
import { isApiUrl } from "@/lib/api-path";
...
describe("ApiClient — envelope de sucesso RF-05 ({ data, message })", () => {
  ...
  it("desembrulha `data` de resposta 2xx em /api/*", async () => {
    ...
    const result = await client.request<{ id: string }>("/cycles/c1");
  ...
  it("expõe o message.code da mutação via messageCodeOf(data)", async () => {
    ...
    const client = new ApiClient("http://api.local");
    const result = await client.post<{ id: string }>("/cycles", { id: "c1" });
  ...
  it("GET sem message não expõe code", async () => {
    ...
    const client = new ApiClient("http://api.local");
    const result = await client.request<{ id: string }[]>("/cycles");
    expect(result).toEqual([{ id: "c1" }]);
  ...
  it("payload com chave `data` própria do domínio não é confundido com envelope", async () => {
    ...
    const client = new ApiClient("http://api.local");
    const result = await client.request<{ data: { x: number }; extra: boolean }>("/misc");
  ...
  it("URL fora do prefixo da API não conta como chamada de API para o desembrulho", () => {
    expect(isApiUrl("http://api.local/health")).toBe(false);
    expect(isApiUrl("http://api.local/metrics")).toBe(false);
    expect(isApiUrl(new ApiClient("http://api.local").urlOf("/cycles/c1"))).toBe(true);
  });

  it("204 continua devolvendo undefined", async () => {
    fetchMock.mockResolvedValue(new Response(null, { status: 204 }));
    const client = new ApiClient("http://api.local");
    await expect(client.del("/cycles/c1")).resolves.toBeUndefined();
  });
  ...
  it("erro não-envelopado segue intacto no ApiError", async () => {
    ...
    const client = new ApiClient("http://api.local");
    const failure = await client.request("/cycles/x").catch((e: unknown) => e);
